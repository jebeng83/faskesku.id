...
class DataObyektifController extends Controller
{
    protected function tableAvailable(): bool
    {
        return Schema::hasTable('sdki_data_obyektif');
    }

    protected function getColumns(): array
    {
        try {
            return Schema::getColumnListing('sdki_data_obyektif');
        } catch (\Throwable $e) {
            return [];
        }
    }
    ...
    public function index(Request $request)
    {
        if (! $this->tableAvailable()) {
            return response()->json(['ok' => false, 'message' => 'tabel sdki_data_obyektif tidak tersedia'], 404);
        }
        ...
        $rows = DB::table('sdki_data_obyektif');
        ...
        $list = $rows->orderBy('kd_oby')->limit(500)->get();

        // Map kolom database ke format yang diharapkan frontend
        $list = $list->map(function ($item) {
            $array = (array) $item;
            // Tambahkan alias kolom untuk kompatibilitas
            if (isset($array['kd_oby']) && ! isset($array['kode'])) {
                $array['kode'] = $array['kd_oby'];
            }
            if (isset($array['kel_obyektif']) && ! isset($array['keluhan'])) {
                $array['keluhan'] = $array['kel_obyektif'];
            }
    ...
    public function store(Request $request)
    {
        ...
        $columnMap = [
            'kode' => Schema::hasColumn('sdki_data_obyektif', 'kd_oby') ? 'kd_oby' : null,
            'keluhan' => Schema::hasColumn('sdki_data_obyektif', 'kel_obyektif') ? 'kel_obyektif' : null,
        ];
        ...
        if ($kodeColumn && isset($data[$kodeColumn])) {
            $exists = DB::table('sdki_data_obyektif')
        ...
        DB::table('sdki_data_obyektif')->insert($data);

        // Ambil data yang baru saja diinsert
        $row = DB::table('sdki_data_obyektif')
            ->where('kd_sdki', $data['kd_sdki'])
            ->where($kodeColumn, $data[$kodeColumn])
            ->first();
        ...
        if ($row) {
            $rowArray = (array) $row;
            if (isset($rowArray['kd_oby']) && ! isset($rowArray['kode'])) {
                $rowArray['kode'] = $rowArray['kd_oby'];
            }
            if (isset($rowArray['kel_obyektif']) && ! isset($rowArray['keluhan'])) {
                $rowArray['keluhan'] = $rowArray['kel_obyektif'];
            }
            $row = (object) $rowArray;
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $cols = $this->getColumns();
        $keyColumn = Schema::hasColumn('sdki_data_obyektif', 'kd_oby') ? 'kd_oby' : 'id';
        ...
        $affected = DB::table('sdki_data_obyektif')
            ->where($keyColumn, $id)
            ->update($payload);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $deleted = DB::table('sdki_data_obyektif')
            ->where($keyColumn, $id)
            ->delete();
