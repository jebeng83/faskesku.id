...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (!Schema::hasTable('satu_sehat_clinicalimpression')) {
            Schema::create('satu_sehat_clinicalimpression', function (Blueprint $table) {
                $table->string('no_rawat', 17);
                $table->date('tgl_perawatan');
                $table->time('jam_rawat');
                $table->enum('status', ['Ralan', 'Ranap']);
                $table->string('id_clinicalimpression', 40)->nullable();
    ...
    {
        Schema::dropIfExists('satu_sehat_clinicalimpression');
    }
};
