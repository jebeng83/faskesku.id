  ...
  isForm();                
    ...
    private void ppLembarObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppLembarObat1ActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            param.put("nip",Sequel.cariIsi("select nip from pasien where no_rkm_medis=?",TNoRm.getText()));
            param.put("kartu",Sequel.cariIsi("select no_peserta from pasien where no_rkm_medis=?",TNoRm.getText()));
            param.put("jk",Sequel.cariIsi("select jk from pasien where no_rkm_medis=?",TNoRm.getText()));
            param.put("hp",Sequel.cariIsi("select no_tlp from pasien where no_rkm_medis=?",TNoRm.getText()));
            param.put("ijindokter",Sequel.cariIsi("select no_ijn_praktek from dokter where kd_dokter=?",KdDokter.getText()));
            ...
            param.put("tglresep",Sequel.cariIsi("select tgl_peresepan from resep_obat where no_rawat=?",TNoRw.getText()));
            param.put("jamresep",Sequel.cariIsi("select jam_peresepan from resep_obat where no_rawat=?",TNoRw.getText()));
