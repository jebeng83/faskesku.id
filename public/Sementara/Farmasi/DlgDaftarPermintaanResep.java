            ...
            if(TabRawatJalan.getSelectedIndex()==0){
                if(tabMode.getRowCount()==0){
                    ...
                    param.put("penanggung",Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?",Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?",NoRawat)));
                     param.put("alamat",Sequel.cariIsi("select almt_pj from reg_periksa where no_rawat=?",NoRawat));
                    param.put("noreg",Sequel.cariIsi("select no_reg from reg_periksa where no_rawat=?",NoRawat));
                    param.put("umur",Sequel.cariIsi("select umur from pasien where no_rkm_medis=?",NoRawat));
                    param.put("nip",Sequel.cariIsi("select nip from pasien where no_rkm_medis=?",NoRawat));
                    param.put("kartu",Sequel.cariIsi("select no_peserta from pasien where no_rkm_medis=?",NoRawat));
                    param.put("jk",Sequel.cariIsi("select jk from pasien where no_rkm_medis=?",NoRawat));
                    param.put("hp",Sequel.cariIsi("select no_tlp from pasien where no_rkm_medis=?",NoRawat));
                    param.put("ijindokter",Sequel.cariIsi("select no_ijn_praktek from dokter where kd_dokter=?",KodeDokter));
                    param.put("diagnosa",Sequel.cariIsi("select concat(diagnosa_pasien.kd_penyakit,' ',penyakit.nm_penyakit) from diagnosa_pasien inner join reg_periksa inner join penyakit "+
                            ...
                            "where diagnosa_pasien.no_rawat=? and diagnosa_pasien.prioritas='1'",NoRawat));
                    param.put("bb",Sequel.cariIsi("select berat from pemeriksaan_ralan where no_rawat=?",NoRawat));
                    param.put("tinggi",Sequel.cariIsi("select tinggi from pemeriksaan_ralan where no_rawat=?",NoRawat));
                    param.put("tensi",Sequel.cariIsi("select tensi from pemeriksaan_ralan where no_rawat=?",NoRawat));
                    param.put("alergi",Sequel.cariIsi("select alergi from pemeriksaan_ralan where no_rawat=?",NoRawat));
                    param.put("tglresep",Sequel.cariIsi("select tgl_peresepan from resep_obat where no_rawat=?",NoRawat));
                    param.put("jamresep",Sequel.cariIsi("select jam_peresepan from resep_obat where no_rawat=?",NoRawat));
                    param.put("poli",Sequel.cariIsi("select nm_poli from poliklinik where kd_poli=?",Sequel.cariIsi("select kd_poli from reg_periksa where no_rawat=?",NoRawat)));
                    param.put("propinsirs",akses.getpropinsirs());
