...
const REFERENSI_CONFIG = {
    ...
    diagnosa: {
        ...
        parse: (json) => {
            ...
            return list.map((it) => {
                ...
                const suffix = hasNs
                    ? ` — ${it.nonSpesialis ? "Non Spesialis" : "Spesialis"}`
                    : "";
                return {
                    value,
                    label: `${value} — ${name}${suffix}`,
                    nonSpesialis: hasNs ? it.nonSpesialis : null,
                };
            });
        },
    },
        ...
        parse: (json) => {
            ...
            return list.map((it) => ({
                value:
                    ...
                    it?.kode ||
                    "",
                label: it?.nmDokter || it?.nmProvider || it?.nama || "",
            }));
        },
    },
    poli: {
        ...
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdPoli || it?.kode || "",
                label: it?.nmPoli || it?.nama || "",
            }));
        },
    },
    kesadaran: {
        ...
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdSadar || it?.kode || "",
                label: it?.nmSadar || it?.nama || "",
            }));
        },
    },
    statuspulang: {
        ...
        buildUrl: ({ rawatInap = false } = {}) =>
            `/api/pcare/statuspulang?rawatInap=${rawatInap ? "true" : "false"}`,
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdStatusPulang || it?.kdStatus || it?.kode || "",
                label: it?.nmStatusPulang || it?.nmStatus || it?.nama || "",
            }));
        },
    },
    prognosa: {
        ...
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdPrognosa || it?.kode || "",
                label: it?.nmPrognosa || it?.nama || "",
            }));
        },
    },
    provider: {
        ...
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdProvider || it?.kode || "",
                label: it?.nmProvider || it?.nama || "",
            }));
        },
    },
    subspesialis: {
        ...
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdSubSpesialis || it?.kode || "",
                label: it?.nmSubSpesialis || it?.nama || "",
            }));
        },
    },
    sarana: {
        ...
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdSarana || it?.kode || "",
                label: it?.nmSarana || it?.nama || "",
            }));
        },
    },
    tindakan: {
        ...
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
                value: it?.kdTindakan || it?.kode || "",
                label: (it?.nmTindakan || it?.nama || "")
                    .toString()
                    .replace(/\r\n/g, " ")
                    .trim(),
            }));
    ...
    alergi: {
        ...
        buildUrl: ({ jenis = "01" } = {}) =>
            `/api/pcare/alergi?jenis=${encodeURIComponent(jenis)}`,
        parse: (json) => {
            const list = json?.response?.list || json?.list || json?.data || [];
            return list.map((it) => ({
    ...
    // ===== Sumber: Data Barang (databarang) untuk pencarian nama obat =====
    // Endpoint: GET /api/barang/search?q=
    databarang: {
        supportsSearch: true,
        minChars: 2,
        defaultParams: { q: "" },
        buildUrl: ({ q = "" } = {}) => {
            const params = new URLSearchParams({ q });
            return `/api/barang/search?${params.toString()}`;
        },
        parse: (json) => {
            const items = Array.isArray(json?.data) ? json.data : [];
            return items.map((it) => {
                const kode = it?.kode_brng || "";
                const nama = it?.nama_brng || "";
                return {
                    value: kode,
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            const clickedInsidePortal =
                portalElRef.current &&
                portalElRef.current.contains(event.target);
    ...
    useEffect(() => {
        ...
        if (isOpen) {
            ...
            if (!portalElRef.current) {
                ...
                portalElRef.current.setAttribute(
