...
import { LayoutDashboard, Settings, Users, NotebookTabs } from "lucide-react";
...
export default function SidebarUser({ title = "Manajemen User", children, wide = false }) {
    const { url } = usePage();

    const [isSidebarOpen, setIsSidebarOpen] = useState(() => {
        if (typeof window === "undefined") return false;
        return localStorage.getItem("userSidebarOpen") === "true";
    });
    ...
    useEffect(() => {
        try {
            localStorage.setItem("userSidebarOpen", String(isSidebarOpen));
        } catch (_) {}
    }, [isSidebarOpen]);

    useEffect(() => {
        try {
            localStorage.setItem("userSidebarCollapsed", String(isSidebarCollapsed));
        } catch (_) {}
    }, [isSidebarCollapsed]);
    ...
    const paths = useMemo(
        () => ({
            ...
            previous: safeRoute("setting.index", "/setting"),
            users: safeRoute("users.index", "/users"),
            permissions: safeRoute("permissions.index", "/permissions"),
        }),
        []
    );
                ...
                <nav className="px-2 py-2 text-white">
                    <Section title="Menu Utama" collapsed={isSidebarCollapsed}>
                        ...
                        <NavItem
                            ...
                            icon={Settings}
                            label="Pengaturan"
                            active={isActive(paths.previous)}
                        />
                    </Section>

                    <Section title="User & Permission" collapsed={isSidebarCollapsed}>
                        <NavItem
                            collapsed={isSidebarCollapsed}
                            href={paths.users}
                            icon={Users}
                            label="Manajemen User"
                            active={isActive(paths.users)}
                        />
                        <NavItem
                            collapsed={isSidebarCollapsed}
                            ...
                            active={isActive(paths.permissions)}
                        />
                    </Section>
                </nav>
            </aside>
