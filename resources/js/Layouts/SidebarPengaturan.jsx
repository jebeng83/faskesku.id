...
function NavItem({ href, icon: Icon, label, active, collapsed }) {
  return (
    <Link
      ...
      className={`group flex ${collapsed ? "justify-center px-2 py-3" : "items-center gap-3 px-3 py-2"} rounded text-sm transition-colors ${
        active
          ? "bg-white/15 text-white ring-1 ring-white/30"
          : "text-white/90 hover:bg-white/10"
      }`}
    >
      {Icon && <Icon className={`${collapsed ? "w-5 h-5" : "w-4 h-4"}`} />}
...
export default function SidebarPengaturan({ title = "Pengaturan", children }) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("pengaturanSidebarOpen", String(isSidebarOpen));
    } catch (_) {}
  }, [isSidebarOpen]);

  useEffect(() => {
    try {
      localStorage.setItem("pengaturanSidebarCollapsed", String(isSidebarCollapsed));
    } catch (_) {}
  }, [isSidebarCollapsed]);
  ...
  const paths = useMemo(() => ({
    ...
    mobileJknSetting: safeRoute("pcare.setting.mobilejkn.index", "/pcare/setting-mobilejkn"),
      ...
      >
        <div className="flex items-center justify-between px-3 py-3 border-b border-white/10">
          <div className="flex items-center gap-2">
            <Link href={paths.dashboard} className="inline-flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            {!isSidebarCollapsed && <ThemeToggle isDark={isDark} setIsDark={setIsDark} />}
            <button
              type="button"
        ...
        <nav className="px-2 py-2 text-white">
          <Section title="Menu Utama" collapsed={isSidebarCollapsed}>
            <NavItem collapsed={isSidebarCollapsed} href={paths.dashboard} icon={LayoutDashboard} label="Dashboard" active={isActive(paths.dashboard)} />
          </Section>

          <Section title="Pengaturan Aplikasi" collapsed={isSidebarCollapsed}>
            <NavItem collapsed={isSidebarCollapsed} href={paths.settingIndex} icon={Settings} label="Setting Aplikasi" active={isActive(paths.settingIndex)} />
          </Section>

          <Section title="Bridging PCare" collapsed={isSidebarCollapsed}>
            <NavItem collapsed={isSidebarCollapsed} href={paths.pcareSetting} icon={LinkIcon} label="Setting Bridging PCare" active={isActive(paths.pcareSetting)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.mobileJknSetting} icon={LinkIcon} label="Setting Mobile JKN" active={isActive(paths.mobileJknSetting)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.pcareMapDokter} icon={Stethoscope} label="Mapping Dokter PCare" active={isActive(paths.pcareMapDokter)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.pcareMapPoli} icon={NotebookTabs} label="Mapping Poli PCare" active={isActive(paths.pcareMapPoli)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.pcareMapObat} icon={Pill} label="Mapping Obat PCare" active={isActive(paths.pcareMapObat)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.jadwalDokter} icon={CalendarDays} label="Jadwal Dokter" active={isActive(paths.jadwalDokter)} />
          </Section>

          <Section title="Bridging Satu Sehat" collapsed={isSidebarCollapsed}>
            <NavItem collapsed={isSidebarCollapsed} href={paths.ssOrg} icon={Building2} label="Mapping Organisasi" active={isActive(paths.ssOrg)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.ssLoc} icon={NotebookTabs} label="Mapping Location" active={isActive(paths.ssLoc)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.ssLocFarmasi} icon={NotebookTabs} label="Mapping Location Farmasi" active={isActive(paths.ssLocFarmasi)} />
            <NavItem collapsed={isSidebarCollapsed} href={paths.ssLocRanap} icon={NotebookTabs} label="Mapping Location Ranap" active={isActive(paths.ssLocRanap)} />
          </Section>
        </nav>
      </aside>
        ...
        <div className="h-full flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <nav className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
              <Link href={paths.dashboard} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                Dashboard
              </Link>
