                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-4">
                        ...
                        {pcarePendaftaran ? (
                          <div className="grid grid-cols-1 gap-y-3 text-sm text-gray-700 dark:text-gray-200">
                            ...
                            <div className="space-y-1"><div className="text-xs font-medium">Status</div><div className={`text-sm ${String(pcarePendaftaran.status||'')==='Terkirim'?'text-emerald-700 dark:text-emerald-300':'text-red-700 dark:text-red-300'}`}>{pcarePendaftaran.status || '-'}</div></div>
                            ...
                            <div className="pt-2 flex items-center gap-2">
                              ...
                              <button type="button" onClick={async () => { try { const res = await fetch(`/api/pcare/pendaftaran/rawat/${encodeURIComponent(noRawat)}`, { headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'include' }); const js = await res.json(); const data = js && js.data ? js.data : null; setPcarePendaftaran(data); } catch {} try { if (typeof window !== 'undefined' && typeof window.__refreshBridgingVisible === 'function') { window.__refreshBridgingVisible(); } } catch {} }} className="inline-flex items-center px-3 py-1.5 text-sm rounded-md bg-neutral-900 hover:bg-neutral-800 text-white border border-neutral-700">Perbarui Status</button>
                            </div>
                          </div>
                        ) : (
