      ...
      >
        ...
        <div className="relative p-6 md:p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            ...
            <Field label="Pencarian (No Rawat/RM/Nama)" icon={Search}>
              <input
                placeholder="Cari…"
                className="w-full rounded-lg border border-gray-300/50 dark:border-gray-600/50 bg-white/80 dark:bg-gray-700/80 backdrop-blur-sm px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 dark:focus:border-blue-400 transition-all duration-200"
                value={q}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 mt-4 md:mt-6">
            <Field label="Filter Penjamin" icon={CreditCard}>
              <input
            ...
            <Field label="Filter Dokter/Petugas" icon={User}>
              <input
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mt-4 md:mt-6">
            <Field label="Status Registrasi" icon={FileText}>
              <select
                className="w-full rounded-lg border border-gray-300/50 dark:border-gray-600/50 bg-white/80 dark:bg-gray-700/80 backdrop-blur-sm px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 dark:focus:border-blue-400 transition-all duration-200"
                value={status}
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-between mb-4">
        ...
        >
          ...
          <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">
            Total data: <span className="text-blue-600 dark:text-blue-400">{LCount}</span>
          </span>
        </motion.div>
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-gradient-to-r from-gray-50/80 to-gray-100/80 dark:from-gray-800/80 dark:to-gray-900/80 backdrop-blur-sm border-b border-gray-200/50 dark:border-gray-700/50">
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Kode Dokter</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">No. RM</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Poliklinik</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Penanggung Jawab</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Penjamin</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Status</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">No. Rawat</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Tgl</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Jam</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Status Bayar</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">No. Tlp</th>
              ...
              {loading ? (
                ...
                <AnimatePresence>
                  {filtered.map((r, index) => {
                    const statusBadgeClass = r?.status_bayar === 'Sudah Bayar'
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="font-mono text-xs font-semibold text-gray-900 dark:text-gray-100">{r?.kd_dokter || '-'}</span>
                            <span className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{r?.dokter?.nm_dokter || '-'}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="font-mono text-xs font-semibold text-gray-900 dark:text-gray-100">{r?.no_rkm_medis || '-'}</span>
                            <span className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{r?.pasien?.nm_pasien || '-'}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{r?.poliklinik?.nm_poli || '-'}</td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="text-gray-900 dark:text-gray-100">{r?.p_jawab || '-'}</span>
                            <span className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{r?.almt_pj || '-'}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="text-gray-900 dark:text-gray-100">{r?.penjab?.png_jawab || r?.kd_pj || '-'}</span>
                            <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 mt-0.5">{currency.format(Number(r?.biaya_reg || 0))}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <motion.span
                        ...
                        <td className="px-4 py-3 font-medium">
                          <div className="flex flex-col">
                            <span className="font-mono text-xs font-bold text-gray-900 dark:text-gray-100">{r?.no_rawat || '-'}</span>
                            <span className="text-xs text-gray-600 dark:text-gray-400 font-normal mt-0.5">{r?.no_reg || '-'}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{r?.tgl_registrasi || '-'}</td>
                        <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{r?.jam_reg || '-'}</td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <motion.span
                            ...
                            <span className="text-xs text-gray-600 dark:text-gray-400 mt-1">{r?.status_poli || '-'}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="text-gray-900 dark:text-gray-100">{r?.pasien?.no_tlp || '-'}</span>
                            <span className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{r?.keputusan || '-'}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <motion.a
                              href={`/pembayaran/ralan/${encodeURIComponent(r.no_rawat || '')}`}
                              className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white text-xs font-semibold shadow-md shadow-indigo-500/25 hover:shadow-lg hover:shadow-indigo-500/30 transition-all duration-200"
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                            >
                              Detail
                            ...
                            <motion.a
                              href={`/akutansi/billing?no_rawat=${encodeURIComponent(r.no_rawat || '')}`}
                ...
                <tr>
                  <td colSpan={12} className="px-4 py-12 text-center">
                    ...
                    >
                      ...
                      <span className="text-sm font-semibold text-gray-600 dark:text-gray-400">Tidak ada data untuk filter saat ini.</span>
                    </motion.div>
                  </td>
                </tr>
              )}
