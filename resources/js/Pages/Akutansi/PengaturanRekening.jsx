...
const COLS_RANAP2 = ['Persediaan_Obat_Kamar_Operasi_Ranap','Harian_Ranap','Uang_Muka_Ranap','Piutang_Pasien_Ranap','Sisa_Uang_Muka_Ranap'];

const humanize = (key) => (key || '').replace(/_/g, ' ');

function SectionForm({ sectionKey, title, columns, data, onChange, onSave, isSaving }) {
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      ...
      columns.forEach((col) => {
        ...
        tasks.push((async () => {
          try {
            const params = new URLSearchParams({ limit: '10', q: kd });
            const res = await fetch(`/api/akutansi/pengaturan-rekening/rekening?${params.toString()}`, { headers: { Accept: 'application/json' } });
            const json = await res.json();
            const items = Array.isArray(json?.items) ? json.items : [];
            const match = items.find((it) => it?.kd_rek === kd) || items[0] || null;
            if (match) {
              collected[col] = {
      ...
      if (tasks.length > 0) {
        ...
        if (!cancelled) setDetailsByCol((prev) => ({ ...prev, ...collected }));
      }
    };
    doFetch();
    ...
    <Card title={title}>
      <div className="mb-4 flex items-end gap-3">
        <div className="flex-1">
          <label className="block text-xs font-medium text-gray-600 dark:text-gray-400">Filter kolom</label>
          <input
            type="text"
            value={filter}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-3 py-2 text-left font-semibold text-gray-700 border-b">Letak Akun Rekening</th>
              <th className="px-3 py-2 text-left font-semibold text-gray-700 border-b">Kode Akun</th>
              <th className="px-3 py-2 text-left font-semibold text-gray-700 border-b">Nama Akun</th>
              <th className="px-3 py-2 text-left font-semibold text-gray-700 border-b">Tipe</th>
              <th className="px-3 py-2 text-left font-semibold text-gray-700 border-b">Balance</th>
            </tr>
          </thead>
          <tbody>
            {visibleCols.map((col) => {
              ...
              return (
                <tr key={col} className="odd:bg-white even:bg-gray-50">
                  <td className="px-3 py-2 align-middle text-gray-800">{humanize(col)}</td>
                  <td className="px-3 py-2 align-middle">
                    <SearchableSelect
                      source="rekening"
                      value={curVal}
                      onChange={(val) => onChange(sectionKey, { ...(data?.current || {}), [col]: val })}
                      onSelect={(opt) => setDetailsByCol((prev) => ({ ...prev, [col]: opt }))}
                      placeholder="Pilih Rekening"
                      searchPlaceholder="Cari rekening…"
                      defaultDisplay={fallback}
                    />
                  </td>
                  <td className="px-3 py-2 align-middle text-gray-700">{info?.nm_rek || '-'}</td>
                  <td className="px-3 py-2 align-middle text-gray-700">{info?.tipe || '-'}</td>
                  <td className="px-3 py-2 align-middle text-gray-700">{info?.balance || '-'}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="mt-5 flex items-center gap-3">
        ...
        >
          <Save className="w-4 h-4" /> Simpan Perubahan ({changedCount})
        </button>
        <p className="text-xs text-gray-500 dark:text-gray-400">
          Tip: Hanya kolom yang berubah yang akan dikirim ke server. Semua nilai wajib berupa kode rekening (kd_rek) yang valid.
        </p>
      </div>
    </Card>
  );
...
export default function PengaturanRekeningPage() {
  ...
  const loadAll = async () => {
    try {
      ...
      const apply = (key, cols) => {
        const row = data?.[key] || {};
        const original = {};
        cols.forEach((c) => { original[c] = row?.[c] || ''; });
  ...
  const handleChange = (sectionKey, newCurrentOrObj) => {
    setPayload((prev) => ({
      ...
      [sectionKey]: {
        ...
        current: typeof newCurrentOrObj === 'object' ? newCurrentOrObj : { ...(prev[sectionKey]?.current || {}), value: newCurrentOrObj },
      },
    }));
  };
  ...
  const saveSection = async (sectionKey) => {
    ...
    setSaving(true);
    try {
      const res = await axios.put(`/api/akutansi/pengaturan-rekening/${encodeURIComponent(sectionKey)}`, changes);
      const fresh = res.data?.data || {};
      // reset original = fresh, current = fresh
      setPayload((prev) => ({
        ...prev,
        [sectionKey]: {
          original: { ...(prev[sectionKey]?.original || {}), ...fresh },
          current: { ...(prev[sectionKey]?.current || {}), ...fresh },
        },
      }));
    } catch (e) {
      const msg = e?.response?.data?.message || 'Gagal menyimpan. Periksa konsol.';
  ...
  const TabButton = ({ value, label }) => (
    <button
      ...
      className={`px-4 py-2 text-sm font-medium rounded-lg border transition-all ${activeTab === value ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50'}`}
    >
      {label}
    </button>
  );
      ...
      <div className="space-y-6">
        ...
        <Card>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              <h1 className="text-xl font-bold tracking-tight bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 bg-clip-text text-transparent">Pengaturan Rekening / COA</h1>
              <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Map kolom transaksi ke Kode Rekening. Nilai harus berupa kd_rek yang valid.</p>
            </div>
        ...
        <Card>
          <div className="text-xs text-gray-600 dark:text-gray-400 flex items-start gap-2">
            ...
            <div>
              <p>
                Validasi otomatis akan memastikan setiap kolom yang Anda kirim berisi kd_rek yang ada di tabel rekening. Kolom yang tidak Anda ubah tidak akan dikirim.
              </p>
              <p className="mt-1">
                Update dilakukan massal pada satu baris konfigurasi per tabel (tanpa WHERE) sesuai asumsi basis data Khanza. Jika tabel belum memiliki baris, sistem akan menyisipkan baris kosong terlebih dahulu saat penyimpanan.
              </p>
            </div>
          </div>
        </Card>
...
PengaturanRekeningPage.layout = (page) => <SidebarKeuangan title="Keuangan" children={page} />;