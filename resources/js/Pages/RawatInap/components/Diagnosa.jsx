...
export default function Diagnosa({ token = '', noRkmMedis = '', noRawat = '' }) {
    ...
    useEffect(() => {
        const loadSavedDiagnosa = async () => {
            ...
            try {
                ...
                const res = await fetch(`/api/rawat-inap/diagnosa?${params.toString()}`, { headers: { Accept: 'application/json' } });
                const json = await res.json();
                const list = json?.data || [];
                ...
                console.warn('Gagal memuat diagnosa ranap tersimpan:', e?.message);
            }
        };
        loadSavedDiagnosa();
    }, [noRawat]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetch('/api/rawat-inap/diagnosa', {
                method: 'POST',
            ...
            const json = await res.json();
            if (!res.ok) {
                throw new Error(json?.message || 'Gagal menyimpan diagnosa ranap');
            }
            const list = json?.data || [];
            const mapped = list.map((it) => ({
                kode: it.kode,
            ...
            setSelected(mapped);
            setSaveStatus({ type: 'success', message: 'Diagnosa ranap berhasil disimpan' });
        } catch (e) {
            setSaveStatus({ type: 'error', message: e?.message || 'Gagal menyimpan diagnosa ranap' });
        } finally {
