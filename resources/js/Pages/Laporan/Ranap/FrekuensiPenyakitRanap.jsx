...
import { Calendar, Search, PieChart, BarChart, Loader2, Printer, Stethoscope, Wallet, X, BedDouble } from 'lucide-react';
import { 
...
export default function FrekuensiPenyakitRanap({ listBangsal = [], listDokter = [], listPenjab = [] }) {
    ...
    const [dateType, setDateType] = useState('masuk'); // masuk | keluar
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get(route('laporan.ranap.frekuensi-penyakit.data'), {
                params: {
    ...
    useEffect(() => {
        fetchData();
    }, [startDate, endDate, dateType, bangsal, dokter, penjab]);
    ...
    const handlePrint = () => {
        ...
        const url = route('laporan.ranap.frekuensi-penyakit.print') + '?' + params.toString();
    ...
    return (
        <SidebarLaporan title="Laporan">
            <Head title="Frekuensi Penyakit Ranap" />

            <div className="px-4 sm:px-6 lg:px-8 py-6 print:p-0">
                <div className="sm:flex sm:items-center print:hidden">
                    <div className="sm:flex-auto">
                        <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Frekuensi Penyakit Ranap</h1>
                        <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">
                            Laporan frekuensi penyakit rawat inap berdasarkan diagnosa utama pasien.
                        </p>
                    </div>
                </div>
                
                <div className="hidden print:block mb-4 text-center">
                    <h2 className="text-xl font-bold text-gray-900">Laporan Frekuensi Penyakit Rawat Inap</h2>
                    <p className="text-sm text-gray-600">Periode: {startDate} s.d {endDate} ({dateType === 'masuk' ? 'Tgl Masuk' : 'Tgl Keluar'})</p>
                </div>
                             ...
                             <div className="col-span-1 md:col-span-2 lg:col-span-4 flex gap-4">
                                <label className="inline-flex items-center">
                                    <input
                                        ...
                                        name="dateType"
                                        value="masuk"
                                        checked={dateType === 'masuk'}
                                        onChange={(e) => setDateType(e.target.value)}
                                    />
                                    <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Berdasarkan Tanggal Masuk</span>
                                </label>
                                <label className="inline-flex items-center">
                                    <input
                                        type="radio"
                                        ...
                                        name="dateType"
                                        value="keluar"
                                        checked={dateType === 'keluar'}
                                        onChange={(e) => setDateType(e.target.value)}
                                    />
                                    <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Berdasarkan Tanggal Keluar</span>
                                </label>
                            </div>
