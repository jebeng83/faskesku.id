...
// Helper to check if a named route exists in Ziggy
const hasRoute = (name) => Boolean(Ziggy?.routes?.[name]);
...
export default function MenuDashboard({ auth, menus = [] }) {
    ...
    const handleMenuClick = (menu) => {
        if (menu.route && hasRoute(menu.route)) {
            router.visit(route(menu.route, undefined, undefined, Ziggy));
        }
    };

    const renderMenuCard = (menu) => {
        const available = menu.route && hasRoute(menu.route);
        const href = available ? route(menu.route, undefined, undefined, Ziggy) : '#';
        return (
            <Link
                key={menu.id}
                href={href}
                aria-disabled={!available}
                className={`block ${menu.bgColor} border ${menu.borderColor} rounded-xl p-6 transition-all duration-200 hover:shadow-lg hover:scale-105 group ${!available ? 'opacity-60 cursor-not-allowed pointer-events-none' : ''}`}
            >
                <div className="flex flex-col items-center text-center space-y-4">
                    ...
                    <div>
                        ...
                        {!available && (
                            <p className="text-xs text-gray-400 mt-1">Route belum tersedia</p>
                        )}
                    </div>
                </div>
            </Link>
