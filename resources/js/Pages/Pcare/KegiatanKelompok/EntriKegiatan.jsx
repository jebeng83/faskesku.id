...
const ui = {
  label: 'block text-[11px] font-medium text-slate-600 mb-1',
  input: 'w-full rounded-lg border-slate-300 text-sm focus:border-slate-400 focus:ring focus:ring-slate-200/70',
  inputMuted: 'w-full rounded-lg border-slate-200 bg-slate-50 text-sm',
  select: 'w-full rounded-lg border-slate-300 text-sm focus:border-slate-400 focus:ring focus:ring-slate-200/70',
  textarea: 'w-full rounded-lg border-slate-300 text-sm focus:border-slate-400 focus:ring focus:ring-slate-200/70',
  btnPrimary: 'inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-400/60 disabled:opacity-50',
  btnNeutral: 'inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-medium text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50',
  btnSlate: 'inline-flex items-center gap-2 rounded-lg bg-slate-700 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400/60 disabled:opacity-50',
  btnDangerSoft: 'inline-flex items-center rounded-lg bg-rose-50 px-2 py-1 text-[11px] font-medium text-rose-700 ring-1 ring-rose-200 hover:bg-rose-100 disabled:opacity-50',
};
...
export default function EntriKegiatan() {
  ...
  const fetchClubs = async (program) => {
    ...
    setClubId('');
    try {
      const res = await fetch(`/pcare/api/kelompok/club/${encodeURIComponent(program)}`);
      const json = await res.json();
      if (!res.ok) throw new Error(json?.metaData?.message || 'Gagal mengambil data club');
  ...
  const cariPembicara = async () => {
    ...
    try {
      ...
      const res = await fetch(`/pcare/api/peserta/${encodeURIComponent(pembicaraNoKartu)}`);
      const json = await res.json();
      if (!res.ok) throw new Error(json?.metaData?.message || 'Peserta tidak ditemukan');
      const p = json?.response || json?.data || {};
      const nama = p?.nama || json?.response?.peserta?.nama || json?.data?.peserta?.nama || '';
      if (nama) setNamaPembicara(nama);
      else setErrorPembicara('Nama pembicara tidak tersedia pada respons');
    } catch (e) {
  ...
  const fetchRiwayat = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json?.metaData?.message || 'Gagal mengambil riwayat');

      // Dedup kuat: SELALU gunakan kunci gabungan (tglPelayanan + jenisKegiatan + materi), abaikan club.
      // Selain itu, agregasikan nama club ke satu kolom agar pengguna tetap melihat semua club yang terlibat.
      const list = json?.response?.list || [];
      const groups = new Map();
      for (const it of list) {
        const jenisRaw = (it?.kegiatan?.kdKegiatan || it?.kegiatan?.nama || '').toString().trim().toLowerCase();
        const materiRaw = (it?.materi || '').toString().trim().toLowerCase();
        const key = `${it?.tglPelayanan || ''}__${jenisRaw}__${materiRaw}`;

        const clubName = (it?.clubProl?.nama || '').toString().trim();
        if (!groups.has(key)) {
          const clubSet = new Set();
          if (clubName) clubSet.add(clubName);
          groups.set(key, { rep: it, clubs: clubSet });
        } else {
          const g = groups.get(key);
          if (clubName) g.clubs.add(clubName);
        }
      }

      const uniq = Array.from(groups.values()).map(({ rep, clubs }) => {
        const row = { ...rep };
        // simpan gabungan club pada properti "__clubsJoined" untuk dipakai saat render
        row.__clubsJoined = Array.from(clubs).join(', ');
        return row;
      });

      const normalized = { ...json, response: { ...(json.response || {}), list: uniq } };
  ...
  const hapusKegiatan = async (eduId) => {
    ...
    const ok = window.confirm('Yakin ingin menghapus kegiatan ini dari BPJS PCare?');
    if (!ok) return;
    setDeletingEduId(id);
    try {
      const res = await fetch(`/pcare/api/kelompok/kegiatan/${encodeURIComponent(id)}`, {
        method: 'DELETE',
        headers: {
      ...
      if (!res.ok) {
        throw new Error(json?.metaData?.message || json?.message || 'Gagal menghapus kegiatan');
      }
      setRiwayatAction(json?.metaData?.message || 'Kegiatan berhasil dihapus');
      showToast('success', json?.metaData?.message || 'Kegiatan berhasil dihapus');
      // Refresh data riwayat supaya sinkron
      try { await fetchRiwayat(); } catch (_) {}
    } catch (e) {
      setRiwayatActionError(e.message || 'Terjadi kesalahan saat menghapus kegiatan');
      showToast('error', e.message || 'Terjadi kesalahan saat menghapus kegiatan');
    } finally {
      setDeletingEduId('');
    }
  };
  ...
  const masukModeEdit = async (row) => {
    try {
      ...
      const kdKel = String(row?.kdKelompok || row?.kelompok?.kdKelompok || '').trim();
      if (kdKel === '01' || kdKel === '02') setKdProgram(kdKel);

      // club
      const club = row?.clubId ?? row?.clubProl?.clubId ?? row?.clubProl?.kdClub ?? row?.clubProl?.id;
      ...
      const kdKeg = String(row?.kdKegiatan || row?.kegiatan?.kdKegiatan || '').trim();
      ...
      if (typeof row?.biaya !== 'undefined') setBiayaHonor(String(row.biaya));
  ...
  const fetchPesertaByEduId = async (eduId) => {
    ...
    try {
      const res = await fetch(`/pcare/api/kelompok/peserta/${encodeURIComponent(eduId)}`);
      const json = await res.json();
      ...
      setErrorPeserta(e.message || 'Terjadi kesalahan saat mengambil peserta');
      setPesertaRows([]);
    } finally {
  ...
  const tambahPesertaKeBpjs = async () => {
    ...
    if (!eduId) {
      setAddPesertaError('Mohon isi atau pilih Edu ID kegiatan terlebih dahulu.');
      return;
    }
    if (!noka) {
      setAddPesertaError('Mohon isi No. Kartu BPJS peserta terlebih dahulu.');
      return;
    }
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(json?.metaData?.message || json?.message || 'Gagal menambah peserta ke kegiatan');
      }
      setAddPesertaSuccess(json?.metaData?.message || 'Peserta berhasil ditambahkan ke kegiatan');
      // Refresh daftar peserta untuk eduId terkait agar sinkron dengan data BPJS
      if (eduId) {
        try { await fetchPesertaByEduId(eduId); } catch (_) {}
      }
      // Kosongkan input nomor kartu agar siap input berikutnya
      setPesertaNoKartu('');
      setSelectedNoKartu('');
    } catch (e) {
      setAddPesertaError(e.message || 'Terjadi kesalahan saat menambah peserta');
    } finally {
  ...
  const cariPesertaNoKartu = async () => {
    ...
    try {
      const res = await fetch(`/pcare/api/peserta/nokartu/${encodeURIComponent(pesertaNoKartu)}/tgl/${encodeURIComponent(tgl)}`);
      const json = await res.json();
      if (!res.ok) throw new Error(json?.metaData?.message || 'Peserta tidak ditemukan');
  ...
  const hapusPesertaKegiatan = async (noKartu) => {
    ...
    const ok = window.confirm(`Hapus peserta ${noka} dari kegiatan BPJS PCare?`);
    if (!ok) return;

    setDeletingPesertaNoKartu(noka);
    try {
      const res = await fetch(`/pcare/api/kelompok/peserta/${encodeURIComponent(eduId)}/${encodeURIComponent(noka)}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json; charset=utf-8',
          'X-CSRF-TOKEN': getCsrfToken() || undefined,
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(json?.metaData?.message || json?.message || 'Gagal menghapus peserta');
      }
      setPesertaListAction(json?.metaData?.message || 'Peserta berhasil dihapus');
      showToast('success', json?.metaData?.message || 'Peserta berhasil dihapus');
      // Refresh daftar peserta agar sinkron
      try { await fetchPesertaByEduId(eduId); } catch (_) {}
    } catch (e) {
      setPesertaListActionError(e.message || 'Terjadi kesalahan saat menghapus peserta');
      showToast('error', e.message || 'Terjadi kesalahan saat menghapus peserta');
    } finally {
  ...
  return (
    <div className="w-full mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-transparent">Entri Kegiatan Kelompok</h1>
        <p className="mt-1 text-sm text-slate-500">Catat dan kelola kegiatan Prolanis, sekaligus peserta yang hadir.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <motion.div variants={cardVariants} initial="hidden" animate="visible" className="rounded-2xl border border-slate-200/80 bg-white/90 p-5 sm:p-6 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {/* Tanggal Pelaksanaan */}
            <div>
              <label className={ui.label}>Tanggal Pelaksanaan</label>
              <input type="date" value={tglPelaksanaan} onChange={(e) => setTglPelaksanaan(e.target.value)} className={ui.input} />
            </div>

            {/* Jenis Kelompok */}
            <div>
              <label className={ui.label}>Jenis Kelompok</label>
              <select value={kdProgram} onChange={(e) => setKdProgram(e.target.value)} className={ui.select}>
            ...
            <div className="md:col-span-2">
              <label className={ui.label}>Club Prolanis</label>
              <div className="flex gap-2">
                <select value={clubId} onChange={(e) => setClubId(e.target.value)} className={ui.select}>
                  <option value="" disabled>
                    {loadingClub ? 'Memuat club...' : '— Pilih Club Prolanis —'}
                  </option>
                  {clubOptions.map((o) => (
              ...
              {errorClub && <div className="mt-1 text-[11px] text-rose-600">{errorClub}</div>}
            </div>

            {/* Jenis Kegiatan */}
            <div>
              <label className={ui.label}>Jenis Kegiatan</label>
              <select value={jenisKegiatan} onChange={(e) => setJenisKegiatan(e.target.value)} className={ui.select}>
            ...
            <div>
              <label className={ui.label}>Cara penyelenggaraan kegiatan</label>
              <select value={cara} onChange={(e) => setCara(e.target.value)} className={ui.select}>
            ...
            <div>
              ...
              <select value={narasumber} onChange={(e) => setNarasumber(e.target.value)} className={ui.select}>
            ...
            <div>
              ...
              <input type="text" value={materi} onChange={(e) => setMateri(e.target.value)} placeholder="Misal: Aktivitas Fisik (Senam)" className={ui.input} />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex gap-2">
                <input type="text" value={pembicaraNoKartu} onChange={(e) => setPembicaraNoKartu(e.target.value)} placeholder="No Kartu BPJS" className={ui.input} />
                <button onClick={cariPembicara} disabled={loadingPembicara} className={ui.btnSlate}>
                  {loadingPembicara ? (
                    'Cari...'
                  ) : (
                    <>
              ...
              {errorPembicara && <div className="mt-1 text-[11px] text-rose-600">{errorPembicara}</div>}
            </div>

            {/* Nama Pembicara */}
            <div className="md:col-span-2">
              <label className={ui.label}>Nama Pembicara</label>
              <input type="text" value={namaPembicara} onChange={(e) => setNamaPembicara(e.target.value)} className={ui.input} />
            </div>

            {/* Lokasi */}
            <div className="md:col-span-2">
              <label className={ui.label}>Lokasi</label>
              <input type="text" value={lokasi} onChange={(e) => setLokasi(e.target.value)} className={ui.input} />
            </div>

            {/* Biaya */}
            <div>
              <label className={ui.label}>Biaya Honor Narasumber</label>
              <select value={biayaHonor} onChange={(e) => setBiayaHonor(e.target.value)} className={ui.select}>
                <option value="" disabled>— Pilih Biaya Honor —</option>
            ...
            {cara === 'Luring' ? (
              <div className="md:col-span-1">
                <label className={ui.label}>Biaya Konsumsi</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min="0"
                    value={biayaKonsumsi}
                    onChange={(e) => setBiayaKonsumsi(e.target.value)}
                    className={ui.input}
                  />
                  <span className="text-[12px] text-slate-600">X</span>
                  <div className="flex items-center gap-1">
                    <span className="text-[12px] text-slate-600">Jml Peserta</span>
                    <input
                      type="number"
                      min="0"
                      value={jmlPeserta}
                      onChange={(e) => setJmlPeserta(e.target.value)}
              ...
              <div className="md:col-span-1">
                <label className={ui.label}>Biaya Konsumsi</label>
                <input type="number" min="0" value={biayaKonsumsi} disabled className={ui.inputMuted} />
              </div>
            )}

            <div>
              <label className={ui.label}>Total Konsumsi</label>
              <input type="number" min="0" value={totalKonsumsi} readOnly className={`${ui.input} bg-slate-50`} />
            </div>
            <div>
              <label className={ui.label}>Total Biaya</label>
              <input type="number" min="0" value={totalBiaya} readOnly className={`${ui.input} bg-slate-50`} />
            </div>

            {/* Keterangan */}
            <div className="md:col-span-2">
              <label className={ui.label}>Keterangan</label>
              <textarea value={keterangan} onChange={(e) => setKeterangan(e.target.value)} rows={3} className={ui.textarea} />
            </div>
          </div>
          ...
          <div className="mt-4 flex gap-2">
            <button onClick={tambahData} disabled={submitting} className={ui.btnPrimary}>
              {submitting ? (
                'Menyimpan...'
              ) : (
                <>
                  ...
                  <span>{isEditing ? 'Edit Data' : 'Tambah Data'}</span>
                </>
              )}
            </button>
            <button onClick={() => { resetForm(); setIsEditing(false); setEduIdKegiatan(''); setPesertaRows([]); }} className={ui.btnNeutral}>
              <Icon.X className="h-4 w-4" />
              <span>Batal</span>
            </button>
          </div>
        ...
        <motion.div variants={cardVariants} initial="hidden" animate="visible" className="rounded-2xl border border-slate-200/80 bg-white/90 p-5 sm:p-6 shadow-sm">
          <div className="flex items-end gap-3 mb-4">
            <div>
              ...
              <input type="month" value={bulanRiwayat} onChange={(e) => setBulanRiwayat(e.target.value)} className={ui.input} />
            </div>
            <div>
              <button onClick={fetchRiwayat} disabled={loadingRiwayat} className={ui.btnSlate}>
                {loadingRiwayat ? (
                  'Cari...'
                ) : (
                  <>
          ...
          {errorRiwayat && <div className="mb-3 rounded-md border border-rose-200 bg-rose-50 p-2 text-[12px] text-rose-700">{errorRiwayat}</div>}
          {riwayatAction && <div className="mb-3 rounded-md border border-emerald-200 bg-emerald-50 p-2 text-[12px] text-emerald-700">{riwayatAction}</div>}
          {riwayatActionError && <div className="mb-3 rounded-md border border-rose-200 bg-rose-50 p-2 text-[12px] text-rose-700">{riwayatActionError}</div>}
          {riwayat?.metaData && (
            <div className="mb-2 text-[12px] text-slate-600">Meta: {riwayat.metaData.message} (code {riwayat.metaData.code})</div>
          )}
            ...
            <table className="min-w-full text-[12px] border-separate border-spacing-0">
              <thead className="sticky top-0 bg-slate-50/80 backdrop-blur z-10">
                <tr className="text-left text-slate-600 border-b border-slate-200/80">
                  ...
                  <th className="py-2 pr-3">CLUB PROLANIS</th>
                  <th className="py-2 pr-3">MATERI</th>
                  <th className="py-2 pr-3">HAPUS</th>
                </tr>
              </thead>
              <tbody>
                {(riwayat?.response?.list || []).map((row, idx) => (
                  <tr key={row.eduId || idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/70 cursor-pointer odd:bg-white even:bg-slate-50/40" onClick={() => masukModeEdit(row)}>
                    <td className="py-2 pr-3">{row?.kegiatan?.nama || '-'}</td>
                    <td className="py-2 pr-3">{row?.tglPelayanan || '-'}</td>
                    <td className="py-2 pr-3">{row?.__clubsJoined || row?.clubProl?.nama || '-'}</td>
                    <td className="py-2 pr-3">{row?.materi || '-'}</td>
                    <td className="py-2 pr-3">
                      <button
                        className={ui.btnDangerSoft}
                        title="Hapus"
                        onClick={(e) => { e.stopPropagation(); hapusKegiatan(row?.eduId); }}
                        disabled={!row?.eduId || deletingEduId === String(row?.eduId)}
                      >
                        {deletingEduId === String(row?.eduId) ? (
                          'Hapus...'
                        ) : (
                ...
                {(!riwayat || (riwayat?.response?.list || []).length === 0) && (
                  <tr>
                    <td className="py-3 text-center text-slate-400" colSpan={5}>
                      {loadingRiwayat ? 'Memuat...' : 'Belum ada riwayat pada bulan ini'}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="mt-6 rounded-2xl border border-slate-200/80 bg-white/90 p-5 sm:p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="text-base font-semibold text-slate-800">Daftar Hadir Peserta</div>
            {eduIdKegiatan ? (
              <span className="hidden sm:inline-flex items-center rounded-full bg-slate-50 px-2 py-0.5 text-[11px] text-slate-600 ring-1 ring-slate-200">Edu ID: {eduIdKegiatan}</span>
            ) : null}
          </div>
          <button onClick={() => setAddingPeserta(true)} className={ui.btnPrimary}>
            <Icon.Plus className="h-4 w-4" />
        ...
        {pesertaListAction && (
          <div className="mb-2 rounded-md border border-emerald-200 bg-emerald-50 p-2 text-[12px] text-emerald-700">{pesertaListAction}</div>
        )}
        {pesertaListActionError && (
          <div className="mb-2 rounded-md border border-rose-200 bg-rose-50 p-2 text-[12px] text-rose-700">{pesertaListActionError}</div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-900/40 backdrop-blur-sm p-4 sm:p-6 md:p-8 overflow-y-auto overscroll-contain">
            <div className="mt-10 w-full max-w-3xl rounded-2xl border border-slate-200 bg-white shadow-2xl max-h-[85vh] flex flex-col">
              ...
              <div className="flex items-center justify-between border-b px-4 py-3">
                <div className="text-base font-semibold text-slate-800">Peserta Kegiatan Kelompok</div>
                <button
                  onClick={() => {
              ...
              <div className="px-4 py-3 flex-1 overflow-y-auto min-h-0">
                <div className="mb-2">
                  <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-[11px] text-slate-700">Peserta Prolanis</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                  <div className="md:col-span-2">
                    <input
                      type="text"
                      value={eduIdKegiatan}
                      onChange={(e) => setEduIdKegiatan(e.target.value)}
                  ...
                  <div className="md:col-span-3 flex gap-2">
                    <input
                      ...
                      onChange={(e) => setPesertaNoKartu(e.target.value)}
                      placeholder="No. Kartu BPJS"
                      className={ui.input}
                    />
                    <button onClick={cariPesertaNoKartu} disabled={loadingPeserta} className="inline-flex items-center rounded-lg bg-slate-700 px-3 py-2 text-xs font-medium text-white shadow-sm hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400/60 disabled:opacity-50">
                      {loadingPeserta ? (
                        'Cari...'
                      ) : (
                        <>
                ...
                {errorPeserta && <div className="mt-2 text-[11px] text-rose-600">{errorPeserta}</div>}
                {addPesertaError && (
                  <div className="mt-2 rounded border border-rose-200 bg-rose-50 p-2 text-[12px] text-rose-700">{addPesertaError}</div>
                )}
                {addPesertaSuccess && (
                  <div className="mt-2 rounded border border-emerald-200 bg-emerald-50 p-2 text-[12px] text-emerald-700">{addPesertaSuccess}</div>
                )}
                  ...
                  <table className="min-w-full text-[12px] border-separate border-spacing-0">
                    <thead className="sticky top-0 bg-slate-50/80 backdrop-blur z-10">
                      <tr className="text-left text-slate-600 border-b border-slate-200/80">
                        ...
                        <th className="py-2 pr-3 text-center">PILIH</th>
                      </tr>
                    </thead>
                    <tbody>
                      {pesertaRows.map((r) => (
                        <tr key={r.noKartu} className="border-b border-slate-100 last:border-0 odd:bg-white even:bg-slate-50/40 hover:bg-slate-50/70 cursor-pointer">
                          <td className="py-2 pr-3">{r.noKartu}</td>
                          <td className="py-2 pr-3">{r.nama}</td>
                          <td className="py-2 pr-3">{r.sex}</td>
                          <td className="py-2 pr-3">{r.tglLahir}</td>
                          <td className="py-2 pr-3">{r.usia}</td>
                          <td className="py-2 pr-3 text-center">
                            <input
                              type="checkbox"
                              checked={selectedNoKartu === r.noKartu}
                              onChange={(e) => setSelectedNoKartu(e.target.checked ? r.noKartu : '')}
                            />
                          </td>
                        </tr>
                      ))}
                      {pesertaRows.length === 0 && (
                        <tr>
                          <td className="py-3 text-center text-slate-400" colSpan={6}>
                            Belum ada peserta dalam daftar
                          </td>
                        </tr>
                      )}
                    </tbody>
        ...
        <div className="overflow-x-auto rounded-xl ring-1 ring-slate-200">
          <table className="min-w-full text-[12px] border-separate border-spacing-0">
            ...
            <tbody>
              ...
              {!loadingPesertaTable && pesertaRows.map((r) => (
                <tr key={r.noKartu} className="border-b border-slate-100 last:border-0 odd:bg-white even:bg-slate-50/40 hover:bg-slate-50/70">
                  <td className="py-2 pr-3">{r.noKartu}</td>
                  <td className="py-2 pr-3">{r.nama}</td>
                  <td className="py-2 pr-3">{r.sex}</td>
                  <td className="py-2 pr-3">{r.jnsPeserta}</td>
                  <td className="py-2 pr-3">{r.tglLahir}</td>
                  <td className="py-2 pr-3">{r.usia}</td>
                  <td className="py-2 pr-3">{r.prolanis}</td>
                  <td className="py-2 pr-3">
                    <button
                      onClick={() => hapusPesertaKegiatan(r.noKartu)}
                      className={ui.btnDangerSoft}
                      disabled={deletingPesertaNoKartu === r.noKartu}
                    >
                      {deletingPesertaNoKartu === r.noKartu ? (
                        'Hapus...'
                      ) : (
                        <span className="inline-flex items-center gap-1.5">
                          <Icon.X className="h-3.5 w-3.5" />
              ...
              {!loadingPesertaTable && pesertaRows.length === 0 && (
                <tr>
                  <td className="py-3 text-center text-slate-400" colSpan={8}>
...
EntriKegiatan.layout = (page) => <AppLayout title="Entri Kegiatan Kelompok" children={page} />;