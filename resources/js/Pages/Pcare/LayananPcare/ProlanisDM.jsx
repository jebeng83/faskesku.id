...
export default function ProlanisDM() {
  ...
  const doLoad = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/pcare/prolanis/dm/${encoded}/${st}/${lim}`, {
      ...
      if (res.ok && (json?.metaData?.code === 200 || json?.metaData?.code === 201 || !json?.metaData?.code)) {
        ...
        setRaw(json);
        setError(json?.metaData?.message || 'Gagal memuat data Prolanis DM');
      }
    } catch (e) {
          ...
          <div className="px-5 py-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-semibold tracking-tight text-slate-900 dark:text-white">Data Prolanis Diabetes Mellitus (BPJS)</h1>
                  <p className="text-sm text-slate-600 dark:text-slate-400">Mengambil data peserta prolanis DM sejak tahun 2017.</p>
                </div>
              </div>
