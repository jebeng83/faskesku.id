...
export default function CekpesertaPcareKartu() {
  const [noKartu, setNoKartu] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [data, setData] = useState(null);

  const sanitizeNoKartu = (value) => value.replace(/[^0-9]/g, '').slice(0, 16);

  const onChangeNoKartu = (e) => {
    setNoKartu(sanitizeNoKartu(e.target.value));
  };

  const doSearch = async () => {
    const noka = sanitizeNoKartu(noKartu);
    setNoKartu(noka);
    setError(null);
    setData(null);
    if (!noka || noka.length < 8) {
      setError('Masukkan No. Kartu yang valid (min. 8 digit, ideal 13 digit).');
      return;
    }
    setLoading(true);
    try {
      const res = await fetch(`/pcare/api/peserta/${encodeURIComponent(noka)}`, {
          ...
          <div className="px-5 py-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-semibold tracking-tight text-slate-900 dark:text-white">Cek Peserta BPJS PCare (No. Kartu)</h1>
                  <p className="text-sm text-slate-600 dark:text-slate-400">Masukkan nomor kartu untuk melihat informasi peserta melalui layanan PCare.</p>
                </div>
              </div>
      ...
      <motion.div variants={itemVariants} className="relative overflow-hidden rounded-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-gray-200/60 dark:border-gray-700/50 p-4 shadow-xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="md:col-span-2">
            <label htmlFor="noKartu" className="text-xs font-medium text-slate-700">Nomor Kartu Peserta</label>
            <div className="mt-1 flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  inputMode="numeric"
                  autoComplete="off"
                  placeholder="Masukkan No. Kartu (13 digit)"
                  value={noKartu}
                  onChange={onChangeNoKartu}
                  onKeyDown={onKeyDown}
                ...
                />
                {noKartu && (
                  <button onClick={() => setNoKartu('')} className="absolute right-2 top-2 rounded p-1 text-slate-400 hover:text-slate-600" title="Clear">
              ...
              <button onClick={onReset} disabled={loading && !noKartu} className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm hover:bg-slate-50">
                Reset
              </button>
            </div>
          ...
          {data && resp ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      {resp?.jnsPeserta?.nama || 'Peserta'} • {resp?.hubunganKeluarga || '-'} • {aktif ? 'AKTIF' : (resp?.ketAktif || 'TIDAK AKTIF')}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div className="rounded-lg border border-slate-200 p-3">
                  ...
                  <div className="mt-1 text-sm text-slate-800">{resp?.sex || '-'}</div>
                </div>
                <div className="rounded-lg border border-slate-200 p-3">
                ...
                <div className="rounded-lg border border-slate-200 p-3">
                  <div className="text-xs text-slate-500">Prolanis / PRB</div>
                  <div className="mt-1 text-sm text-slate-800">{[resp?.pstProl, resp?.pstPrb].filter(Boolean).join(' / ') || '-'}</div>
                </div>
                <div className="rounded-lg border border-slate-200 p-3">
                  <div className="text-xs text-slate-500">Tunggakan</div>
                  <div className="mt-1 text-sm text-slate-800">{typeof resp?.tunggakan === 'number' ? resp.tunggakan : (resp?.tunggakan ?? '-')}</div>
                </div>
              </div>
            </motion.div>
          ) : (
            ...
            !error && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-center text-slate-500">
                Masukkan nomor kartu dan klik Cari untuk menampilkan data peserta.
              </motion.div>
            )
          )}
        </AnimatePresence>
...
CekpesertaPcareKartu.layout = (page) => (
