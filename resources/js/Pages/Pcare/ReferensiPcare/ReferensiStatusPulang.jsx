...
export default function ReferensiStatusPulang() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [rawatInap, setRawatInap] = useState(false); // default: Rawat Jalan
  ...
  const fetchStatusPulang = async (overrideInap = rawatInap) => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/pcare/statuspulang?rawatInap=${overrideInap ? 'true' : 'false'}`, { headers: { Accept: 'application/json' } });
  ...
  <motion.div variants={itemVariants} className="mb-4">
      ...
      <motion.div variants={itemVariants} className="relative overflow-hidden rounded-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 p-6 shadow-xl shadow-blue-500/5">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="inline-flex items-center rounded-full border border-slate-300 overflow-hidden">
                  <button
                    type="button"
                    onClick={() => setRawatInap(false)}
                    className={`px-3 py-1.5 text-xs ${!rawatInap ? 'bg-slate-100 text-slate-800' : 'bg-white text-slate-600'}`}
                  >Rawat Jalan</button>
                  <button
                    type="button"
                    onClick={() => setRawatInap(true)}
                    className={`px-3 py-1.5 text-xs ${rawatInap ? 'bg-slate-100 text-slate-800' : 'bg-white text-slate-600'}`}
                  >Rawat Inap</button>
                </div>
              </div>
          ...
          <div className="md:col-span-2 flex items-end">
            <button onClick={() => fetchStatusPulang()} className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white px-3 py-2 text-sm shadow transition-all duration-200 hover:from-blue-700 hover:via-indigo-700 hover:to-purple-700">
      ...
      <motion.div variants={itemVariants} className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 p-4 shadow-xl shadow-blue-500/5">
          ...
          <div className="mt-1 text-sm text-slate-800">{rawatInap ? 'Rawat Inap' : 'Rawat Jalan'}</div>
        </div>
        <div className="rounded-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 p-4 shadow-xl shadow-blue-500/5">
          ...
          <AnimatePresence>
            {loading && !hasResult ? (
              ...
              (data.response.list || []).map((item, idx) => (
                <motion.div key={`${item.kdStatusPulang || idx}-${idx}`} variants={itemVariants} initial="hidden" animate="show" exit={{ opacity: 0 }} whileHover={{ scale: 1.01, y: -2 }} className="group relative flex flex-col rounded-2xl border border-white/20 dark:border-gray-700/50 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl p-4 shadow-xl shadow-blue-500/5">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="text-sm font-semibold text-slate-800">{item.kdStatusPulang || '-'}</div>
                      <div className="mt-0.5 text-sm text-slate-700 leading-snug">{item.nmStatusPulang || '-'}</div>
                    </div>
                    <div>
                      {badge(rawatInap ? 'Rawat Inap' : 'Rawat Jalan', 'bg-slate-100 text-slate-700')}
                    </div>
                  </div>
                </motion.div>
              ))
...
ReferensiStatusPulang.layout = (page) => <LayoutUtama title="Briding Pcare" left={<BridingMenu />}>{page}</LayoutUtama>;
