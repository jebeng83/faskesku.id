      ...
      <motion.div variants={itemVariants} className="mb-4">
        <div className="rounded-xl bg-gradient-to-r from-indigo-500 via-sky-500 to-cyan-500 text-white p-5 shadow">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-xl font-semibold">Referensi Dokter PCare</h1>
              <p className="text-sm opacity-90">Daftar dokter dari katalog BPJS PCare.</p>
            </div>
          ...
          <AnimatePresence>
            {loading && !hasResult ? (
              ...
              (data.response.list || []).map((item, idx) => (
                <motion.div key={`${item.kdDokter || item.kdDok || idx}-${idx}`} variants={itemVariants} initial="hidden" animate="show" exit={{ opacity: 0 }} className="group relative flex flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md">
                  <div className="flex items-start justify-between">
                    <div>
...
ReferensiDokter.layout = (page) => <AppLayout title="Referensi Dokter" children={page} />;