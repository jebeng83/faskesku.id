...
export default function ReferensiSarana() {
  ...
  const filteredList = query
    ? list.filter((item) => {
        const k = String(item?.kdSarana || '').toLowerCase();
        const n = String(item?.nmSarana || '').toLowerCase();
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="p-4">
      ...
      <motion.div variants={itemVariants} className="mb-4">
        <div className="rounded-xl bg-gradient-to-r from-sky-500 via-indigo-600 to-violet-600 text-white p-5 shadow">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-xl font-semibold">Referensi Sarana PCare</h1>
              <p className="text-sm opacity-90">Daftar sarana dari katalog BPJS PCare.</p>
            </div>
      ...
      <motion.div variants={itemVariants} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="w-full md:w-80">
            <label className="block text-xs text-slate-500 mb-1">Cari sarana</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Cari sarana (kode/nama)…"
          ...
          <AnimatePresence>
            {loading && !hasResult ? (
              ...
              pageItems.map((item, idx) => (
                <motion.div key={`${item.kdSarana || idx}-${idx}`} variants={itemVariants} initial="hidden" animate="show" exit={{ opacity: 0 }} className="group relative flex flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="text-sm font-semibold text-slate-800">{item.kdSarana || '-'}</div>
                      <div className="mt-0.5 text-sm text-slate-700 leading-snug">{item.nmSarana || '-'}</div>
                    </div>
                    <div className="flex items-center gap-1">
                      {badge('Sarana', 'bg-slate-100 text-slate-700')}
...
ReferensiSarana.layout = (page) => <AppLayout title="Referensi Sarana" children={page} />;