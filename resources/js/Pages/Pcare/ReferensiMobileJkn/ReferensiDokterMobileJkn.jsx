...
export default function ReferensiDokterMobileJkn() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((it) =>
      (it.namadokter || '').toLowerCase().includes(q) ||
      String(it.kodedokter ?? '').toLowerCase().includes(q) ||
      (it.jampraktek || '').toLowerCase().includes(q)
    );
  }, [list, query]);
  ...
  const fetchDokter = async () => {
    if (!kodepoli) {
      setList([]);
      setError('Masukkan kode poli terlebih dahulu');
      return;
    }
    setLoading(true);
    setError('');
    try {
      const params = new URLSearchParams({ tanggal, kodepoli });
      const res = await fetch(`/api/mobilejkn/ref/dokter?${params.toString()}`, {
        headers: { Accept: 'application/json' },
      });
      const json = await res.json();
      // Bentuk respons:
      // 1) { metadata, response: { list: [ ... ] } }
      // 2) { metadata, response: [ ... ] }
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="p-4">
      ...
      <motion.div variants={itemVariants} className="mb-4">
        <div className="rounded-xl bg-gradient-to-r from-emerald-500 via-teal-600 to-green-600 text-white p-5 shadow">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-xl font-semibold">Referensi Dokter Mobile JKN</h1>
              <p className="text-sm opacity-90">Daftar dokter per poli dari katalog BPJS Mobile JKN.</p>
            </div>
      ...
      {/* Card: Total Item + Status + Tanggal + Kode Poli + Pencarian */}
      <motion.div variants={itemVariants} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="w-full md:w-56">
            <label className="block text-xs text-slate-500 mb-1">Kode Poli</label>
            <input
              type="text"
              value={kodepoli}
              onChange={(e) => setKodepoli(e.target.value)}
              placeholder="Mis. 001"
              className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 placeholder:text-slate-400 focus:border-emerald-500 focus:ring-emerald-500"
            />
          </div>

          {/* Search */}
          <div className="w-full md:w-80">
            <label className="block text-xs text-slate-500 mb-1">Cari dokter</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Nama/kode/jam praktek…"
      ...
      <motion.div variants={itemVariants} className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        <AnimatePresence>
          {paginated.map((dr, idx) => (
            <motion.div
              key={`${dr.kodedokter}-${dr.namadokter}-${idx}`}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="text-sm font-semibold text-slate-800">{dr.namadokter || '-'}</div>
                <span className="text-[10px] rounded-md bg-emerald-50 text-emerald-700 px-2 py-0.5 border border-emerald-200">{dr.kodedokter ?? '-'}</span>
              </div>
              <div className="mt-1 text-xs text-slate-600">
                {dr.jampraktek ? (
                  <>Jam: <span className="font-medium text-slate-700">{dr.jampraktek}</span></>
                ) : (
                  <span className="text-slate-500">—</span>
                )}
              </div>
              <div className="mt-0.5 text-xs text-slate-600">
                Kapasitas: <span className="font-medium text-slate-700">{dr.kapasitas ?? '-'}</span>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
...
ReferensiDokterMobileJkn.layout = (page) => <LayoutUtama title="Briding Pcare" left={<BridingMenu />}>{page}</LayoutUtama>;
