...
const PageHeader = () => (
  <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 p-6 text-white shadow-lg">
    <div className="relative z-10">
      <h1 className="text-2xl font-bold tracking-tight">Data Suplier</h1>
      <p className="mt-1 opacity-90">Kelola data suplier dengan cepat dan mudah.</p>
    </div>
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 0.2 }}
  ...
  <AnimatePresence>
    {open && (
      ...
      >
        ...
        >
          <h3 className="text-lg font-semibold">Hapus Suplier</h3>
          <p className="mt-2 text-sm text-gray-600">
            Anda yakin ingin menghapus <span className="font-medium text-gray-900">{item?.nama_suplier}</span> ({item?.kode_suplier})?
          </p>
...
export default function DataSuplierPage() {
  const { props } = usePage();
  ...
  const openCreate = () => {
    ...
    setIsEdit(false);
    setSelected(null);
    setData('kode_suplier', (nextCode || '').toUpperCase());
    setModalOpen(true);
  };
  ...
  const submitForm = (e) => {
    e.preventDefault();
    if (isEdit && selected) {
      if (!selected?.kode_suplier) {
        alert('Kode suplier kosong. Pilih data yang benar sebelum menyimpan perubahan.');
        return;
      }
      const kodeSuplier = selected.kode_suplier.trim();
      const namedUpdateUrl = route('farmasi.datasuplier.update', { kode_suplier: kodeSuplier });

      // Gunakan method spoofing via POST agar aman untuk Laravel
      router.post(namedUpdateUrl, { ...data, _method: 'PUT' }, {
        ...
        onError: (errors) => {
          console.error('Update suplier gagal:', errors);
        }
      });
    } else {
      // Create suplier
      post(route('farmasi.datasuplier.store'), {
        preserveScroll: true,
  ...
  const onSearch = (e) => {
    e.preventDefault();
    router.get(route('farmasi.datasuplier.index'), { q: query, perPage }, { preserveState: true, replace: true });
  };

  const onChangePerPage = (e) => {
    const val = Number(e.target.value);
    setPerPage(val);
    router.get(route('farmasi.datasuplier.index'), { q: query, perPage: val }, { preserveState: true, replace: true });
  };
          ...
          {modalOpen && (
            <div className="mb-5 rounded-lg border border-indigo-200 bg-indigo-50/40 p-4">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold">{isEdit ? 'Edit Suplier' : 'Tambah Suplier'}</h3>
              ...
              <form onSubmit={submitForm} className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <Input
                  ...
                  onChange={(e) => setData('kode_suplier', e.target.value.toUpperCase())}
                  maxLength={5}
                  placeholder="Mis. SUP01"
                  disabled={isEdit}
                  error={errors?.kode_suplier}
                />
                <Input
                  label="Nama Suplier"
                  id="nama_suplier"
            ...
            <form onSubmit={onSearch} className="flex w-full max-w-xl items-center gap-2">
              <div className="flex-1">
                ...
                <input
                  ...
                  placeholder="Cari kode/nama/alamat/kota/no telp/bank/rekening..."
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-600">Nama Suplier</th>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-600">Bank</th>
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-600">Rekening</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items?.data?.length ? items.data.map((item) => (
                  <tr key={item.kode_suplier} className="hover:bg-gray-50">
                    <td className="px-4 py-2 text-sm font-mono">{item.kode_suplier}</td>
                    <td className="px-4 py-2 text-sm">{item.nama_suplier}</td>
                    ...
                    <td className="px-4 py-2 text-sm">{item.nama_bank}</td>
                    <td className="px-4 py-2 text-sm">{item.rekening}</td>
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-6 text-center text-sm text-gray-500">Tidak ada data.</td>
                  </tr>
                )}
              </tbody>
            </table>
