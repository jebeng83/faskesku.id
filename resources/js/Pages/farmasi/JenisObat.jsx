...
const PageHeader = () => (
  <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 p-6 text-white shadow-lg">
    <div className="relative z-10">
      <h1 className="text-2xl font-bold tracking-tight">Jenis Obat</h1>
      <p className="mt-1 opacity-90">Kelola data jenis obat dengan mudah dan cepat.</p>
    </div>
    ...
    {open && (
      <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} transition={{ type: 'spring', stiffness: 260, damping: 20 }} className="mx-4 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
          <h3 className="text-lg font-semibold">Hapus Jenis Obat</h3>
          <p className="mt-2 text-sm text-gray-600">
            Anda yakin ingin menghapus <span className="font-medium text-gray-900">{item?.nama}</span> ({item?.kdjns})?
          </p>
...
export default function JenisObatPage() {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    if (isEdit && selected) {
      const kode = selected.kdjns?.trim();
      if (!kode) {
        alert('Kode jenis kosong. Pilih data yang benar.');
        return;
      }
      const updateUrl = route('farmasi.jenis-obat.update', { kdjns: kode });
      ...
      post(route('farmasi.jenis-obat.store'), {
        preserveScroll: true,
        onSuccess: () => setPanelOpen(false)
      });
    }
  ...
  const onSearch = (e) => {
    e.preventDefault();
    router.get(route('farmasi.jenis-obat.index'), { q: query, perPage }, { preserveState: true, replace: true });
  };

  const onChangePerPage = (e) => {
    const val = Number(e.target.value);
    setPerPage(val);
    router.get(route('farmasi.jenis-obat.index'), { q: query, perPage: val }, { preserveState: true, replace: true });
  };
        ...
        <div className="rounded-xl bg-white p-4 shadow-sm">
          {panelOpen && (
            <div className="mb-5 rounded-lg border border-indigo-200 bg-indigo-50/40 p-4">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold">{isEdit ? 'Edit Jenis Obat' : 'Tambah Jenis Obat'}</h3>
              ...
              <form onSubmit={submitForm} className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                <Input
                  ...
                  id="kdjns"
                  value={data.kdjns}
                  onChange={(e) => setData('kdjns', e.target.value.toUpperCase())}
                  maxLength={4}
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={openCreate} className="mt-6 rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-700">+ Tambah Jenis</button>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-600">Kode Jenis</th>
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-600">Nama</th>
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-600">Keterangan</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items?.data?.length ? items.data.map((item) => (
                  <tr key={item.kdjns} className="hover:bg-gray-50">
                    <td className="px-4 py-2 text-sm font-mono">{item.kdjns}</td>
...
JenisObatPage.layout = (page) => <AppLayout title="Farmasi" children={page} />;