...
export default function GolonganObatPage() {
  ...
  const openCreate = () => {
    ...
    setIsEdit(false);
    setSelected(null);
    // Set kode otomatis dari backend (mis. G01) dan nonaktifkan editing
    setData('kode', (nextCode || 'G01').toUpperCase());
    setPanelOpen(true);
  };
  ...
  const submitForm = (e) => {
    ...
    if (isEdit && selected) {
      ...
      if (!kode) {
        alert('Kode golongan kosong. Pilih data yang benar.');
        return;
      }
      const updateUrl = route('farmasi.golongan-obat.update', { kode });
      ...
      post(route('farmasi.golongan-obat.store'), {
        preserveScroll: true,
        onSuccess: () => setPanelOpen(false)
      });
    }
  ...
  const onSearch = (e) => {
    e.preventDefault();
    router.get(route('farmasi.golongan-obat.index'), { q: query, perPage }, { preserveState: true, replace: true });
  };

  const onChangePerPage = (e) => {
    const val = Number(e.target.value);
    setPerPage(val);
    router.get(route('farmasi.golongan-obat.index'), { q: query, perPage: val }, { preserveState: true, replace: true });
  };

  return (
    <div>
      <Head title="Golongan Obat" />
...
GolonganObatPage.layout = (page) => <SidebarFarmasi title="Farmasi" children={page} />;
