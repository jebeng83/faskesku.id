...
const PageHeader = () => (
  <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 p-6 text-white shadow-lg">
    <div className="relative z-10">
      <h1 className="text-2xl font-bold tracking-tight">Metode Racik</h1>
      <p className="mt-1 opacity-90">Kelola data metode racik obat dengan cepat dan mudah.</p>
    </div>
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 0.2 }}
  ...
  <AnimatePresence>
    {open && (
      ...
      >
        ...
        >
          <h3 className="text-lg font-semibold">Hapus Metode Racik</h3>
          <p className="mt-2 text-sm text-gray-600">
            Anda yakin ingin menghapus <span className="font-medium text-gray-900">{item?.nm_racik}</span> ({item?.kd_racik})?
          </p>
...
export default function MetodeRacikPage() {
  const { props } = usePage();
  ...
  const openCreate = () => {
    ...
    setIsEdit(false);
    setSelected(null);
    setData('kd_racik', (nextCode || '').toUpperCase());
    setModalOpen(true);
  };
  ...
  const submitForm = (e) => {
    e.preventDefault();
    if (isEdit && selected) {
      const kode = (selected?.kd_racik || '').trim();
      if (!kode) {
        alert('Kode racik kosong. Pilih data yang benar sebelum menyimpan perubahan.');
        return;
      }
      const namedUpdateUrl = route('farmasi.metode-racik.update', { kd_racik: kode });
      ...
      post(route('farmasi.metode-racik.store'), {
        preserveScroll: true,
        onSuccess: () => setModalOpen(false)
      });
    }
  ...
  const onSearch = (e) => {
    e.preventDefault();
    router.get(route('farmasi.metode-racik.index'), { q: query, perPage }, { preserveState: true, replace: true });
  };

  const onChangePerPage = (e) => {
    const val = Number(e.target.value);
    setPerPage(val);
    router.get(route('farmasi.metode-racik.index'), { q: query, perPage: val }, { preserveState: true, replace: true });
  };

  return (
    <div>
      <Head title="Metode Racik" />
          ...
          {modalOpen && (
            <div className="mb-5 rounded-lg border border-emerald-200 bg-emerald-50/40 p-4">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold">{isEdit ? 'Edit Metode Racik' : 'Tambah Metode Racik'}</h3>
              ...
              <form onSubmit={submitForm} className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <Input
                  ...
                  id="kd_racik"
                  value={data.kd_racik}
                  onChange={(e) => setData('kd_racik', e.target.value.toUpperCase())}
                  maxLength={3}
                  placeholder="Mis. 001"
                  disabled={isEdit}
                  error={errors?.kd_racik}
                />
                <Input
                  label="Nama Metode Racik"
                  id="nm_racik"
                  value={data.nm_racik}
                  onChange={(e) => setData('nm_racik', e.target.value)}
                  maxLength={30}
                  placeholder="Nama metode racik"
                  error={errors?.nm_racik}
                />
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <form onSubmit={onSearch} className="flex w-full max-w-xl items-center gap-2">
              <div className="flex-1">
                ...
                <input
                  ...
                  placeholder="Cari kode/nama metode racik..."
                  className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-emerald-300"
                />
              </div>
              <button type="submit" className="mt-6 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">
                Cari
              </button>
            </form>
            <div className="flex items-center gap-3">
              <div>
                ...
                <select value={perPage} onChange={onChangePerPage} className="mt-1 rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-emerald-300">
                  {[10, 20, 50].map(n => <option key={n} value={n}>{n}</option>)}
                </select>
              </div>
              <button onClick={openCreate} className="mt-6 rounded-lg bg-teal-600 px-4 py-2 text-sm font-medium text-white hover:bg-teal-700">
                + Tambah Metode Racik
              </button>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-600">Nama Metode Racik</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items?.data?.length ? items.data.map((item) => (
                  <tr key={item.kd_racik} className="hover:bg-gray-50">
                    <td className="px-4 py-2 text-sm font-mono">{item.kd_racik}</td>
                    <td className="px-4 py-2 text-sm">{item.nm_racik}</td>
                    <td className="px-4 py-2 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => openEdit(item)} className="rounded-lg border border-emerald-200 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-50">
                          Edit
                        </button>
...
MetodeRacikPage.layout = (page) => <SidebarFarmasi title="Farmasi" children={page} />;